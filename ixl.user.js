// ==UserScript==
// @name         IXL Bridge (Unipath)
// @namespace    https://unipath.app/
// @version      0.1.0
// @description  Captures IXL exercise activity and auto-sends to khan-bridge Worker (shared with KA bridge). No manual export needed.
// @match        https://*.ixl.com/*
...
(function () {
  ...
  const PENDING_KEY = 'ixl-bridge-pending';
  const STATS_KEY = 'ixl-bridge-stats';
  ...
  async function flush() {
    ...
    try {
      ...
      stats.failed++;
      persistStats();
      console.warn('[IXL Bridge] flush failed, retry next tick:', err.message || err);
    } finally {
      flushing = false;
    }
  }
  ...
  function ensureBadge() {
    ...
    badgeEl.id = 'ixl-bridge-badge';
    Object.assign(badgeEl.style, {
      position: 'fixed', bottom: '8px', right: '8px', zIndex: '99999',
      background: 'rgba(20,20,20,0.78)', color: '#ffd279',
  ...
  function updateBadge() {
    ...
    if (!el) return;
    el.textContent = `🦊 IXL: ${stats.sent} sent · ${pending.length} pending`;
  }
  ...
  window.ixlBridgeStatus = () => {
    const s = { pending: pending.length, sent: stats.sent, failed: stats.failed, lastFlush: stats.lastFlush };
    console.log('[IXL Bridge]', s);
    return s;
  };
  window.ixlBridgeFlush = () => flush();
  window.ixlBridgeClear = () => {
    pending = [];
    persistPending();
    updateBadge();
    console.log('[IXL Bridge] cleared pending');
  };

  console.log('[IXL Bridge] active. Worker:', WORKER_URL, 'Commands: ixlBridgeStatus(), ixlBridgeFlush(), ixlBridgeClear()');
})();
