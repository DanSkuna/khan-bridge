// ==UserScript==
// @name         Khan Academy Bridge (Unipath)
// @namespace    https://unipath.app/
// @version      0.2.0
// @description  Captures KA exercise activity and auto-sends to khan-bridge Worker. No manual export needed.
...
(function () {
  ...
  const STATS_KEY = 'ka-bridge-stats';
  ...
  async function flush() {
    ...
    try {
      ...
      stats.failed++;
      persistStats();
      console.warn('[KA Bridge] flush failed, retry next tick:', err.message || err);
    } finally {
      flushing = false;
    }
  }
  ...
  function ensureBadge() {
    ...
    badgeEl.id = 'ka-bridge-badge';
    Object.assign(badgeEl.style, {
      position: 'fixed', bottom: '8px', right: '8px', zIndex: '99999',
      background: 'rgba(20,20,20,0.78)', color: '#9eff9e',
  ...
  function updateBadge() {
    ...
    if (!el) return;
    el.textContent = `🦦 KA: ${stats.sent} sent · ${pending.length} pending`;
  }
  ...
  window.kaBridgeStatus = () => {
    const s = { pending: pending.length, sent: stats.sent, failed: stats.failed, lastFlush: stats.lastFlush };
    console.log('[KA Bridge]', s);
    return s;
  };
  window.kaBridgeFlush = () => flush();
  window.kaBridgeClear = () => {
    pending = [];
    persistPending();
    updateBadge();
    console.log('[KA Bridge] cleared pending');
  };

  console.log('[KA Bridge] active. Worker:', WORKER_URL, 'Commands: kaBridgeStatus(), kaBridgeFlush(), kaBridgeClear()');
})();
